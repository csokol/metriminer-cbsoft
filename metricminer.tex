...
\section{Principais funcionalidades} % descrição das principais funcionalidades;
    ...
    Apache com mais seis mil \textit{commits}. Nesta tela, são exibidas informações básicas sobre o projeto como número de \textit{commits},
...
\section{Ferramentas relacionadas} %breve comparação com outras ferramentas relacionadas existentes;
    ...
    \subsection{Sonar}
        Não se conhece ferramentas web de suporte a mineração de repositórios de \textit{software}. Uma ferramenta que se assemelha
    ...
    \subsection{Eclipse Metrics}
        O Eclipse Metrics\footnote[2]{http://metrics.sourceforge.net/} é um \textit{plugin} para o Eclipse que calcula uma variedade 
        de métricas de código no ambiente do desenvolvedor. Dessa forma, para que se acompanhe a evolução do código do 
        projeto é necessário executar manualmente o \textit{plugin} para os \textit{releases} que se deseja analisar.

\section{Conclusão}
    O MetricMiner é uma ferramenta que agrega valor a área de mineração de respositórios de \textit{software}, armazenando um grande volume
