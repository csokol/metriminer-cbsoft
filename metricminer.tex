...
\section{Arquitetura} % arquitetura de solução da ferramenta;
    ...
    O MetricMiner se baseia na simulação de uma fila de execução para completar tarefas demoradas, como clonar
    ...
    dados, constrói os objetos necessários para a execução da tarefa e a executa.
    
    O processo de mineração dos dados e cálculo das métricas implementado pode ser observado no diagrama da Figura
    \ref{fig:diagrama}. Cada tarefa representada por uma classe executa uma etapa do processo.
    Ao final, todas as informações do projeto estão persistidas no banco de dados e disponíveis
    ...
    A arquitetura é bastante flexível na definição de novas tarefas, é necessário apenas que sejam desenvolvidas
    ...
    linha de comando que extrai dados de um repositório local e persiste em banco de dados relacional. Com isso,
    o componente que faz a interface com o sistema de controle de versões pôde ser reutilizado no MetricMiner. 
    Esse componente é bastante flexível em relação ao sistema de controle de versão utilizado pelo projeto que será minerado.
    Para suportar um novo sistema de controle de versão é necessário apenas a implementação da intreface org.metricminer.SCM, que contém 
    operações básicas sobre os metadados do sistema de controle de versão. Até o momento, já foi implementado suporte
    ao GIT.
