...
operações básicas sobre os metadados do sistema de controle de versão. Até o momento, o MetricMiner suporta 
apenas o git. (referencias git, revolution)
...
e uma classe que implemente a interface org.metricminer.tasks.metric.common.Metric. Para calcular essas métricas
sobre o código dos projetos cadastrados, utiliza-se a classe org.metricminer.CalculateMetricTask que ao ser executada na fila de
execução, calcula uma determinada métrica para todas as versões de todos os códigos fonte de um projeto. Até o 
momento já foram implementadas as métricas: 
\begin{itemize}
...
\section{Principais funcionalidades} % descrição das principais funcionalidades;
    ...
    foram calculadas para o projeto. O usuário pode selecionar as métricas que deseja calcular e as tarefas para os cálculos serão
    inseridas à fila de execução. Na seção "Scheduled Tasks", as tarefas que já foram executadas podem ser vizualizadas. Nesse caso,
    podemos ver que as métricas de complexidade ciclomática e a fan-out já foram calculadas, enquanto a LCOM e a quantidade de 
    invocações de métodos estão na fila de execução para serem calculadas.
